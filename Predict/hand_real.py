...
pickle_in = open("model_trained_hand_new.p","rb")
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                denemeImage = denemeImage.crop((left-50, top-50, right+50, bottom+50)) # frame i crop ettim
                
                denemeImage=np.array(denemeImage) #np.uint8() # Hata önlemek ve modele uygun veri hali
                
                gray = preProcess(denemeImage) # kullanılan gri tonlu frame (eski)
                ...
                #gray = gray.resize(grayX+100, grayY+100)
#                denemeImage = cv.cvtColor(denemeImage,cv.COLOR_BGR2GRAY)
#                adaptiveThreshold = cv.adaptiveThreshold(denemeImage,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY,11,2)
                ...
                cv.imshow("modele giden",thresh)
                
                #img = np.array(gray)
                
                img = thresh
                
                #img = cv.GaussianBlur(img,ksize=(3,3),sigmaX=7)
                
                img = cv.resize(img, (224, 224)) # input shape
                
                #img = cv.cvtColor(img, cv.COLOR_BGR2GRAY) #--> prediction for denemeImage
                
                #img = cv.equalizeHist(img)
                #img = img / 255 
        
                img = img.reshape(1,224, 224,1) # 1 adet resim, (120, 320) boytunda, channel = 1
                ...
                if probVal > 0.7:
                    if classIndex == 0:
                        ...
                        cv.putText(image, "Yumruk"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
                    elif classIndex == 2:
                        cv.putText(image, "L"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
                    elif classIndex == 3:
                        cv.putText(image, "Okay"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
                    elif classIndex == 4:
                        cv.putText(image, "Avuc"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
                    elif classIndex == 5:
                        cv.putText(image, "Peace"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
...
#                        cv.putText(image, "Yumruk"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
#                    elif classIndex == 3:
#                        cv.putText(image, "Bas parmak"+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
#                    elif classIndex == 4:
