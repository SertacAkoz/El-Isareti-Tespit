...
pickle_in = open("sign_language_model.p","rb")
model = pickle.load(pickle_in) 

predictList = ["Merhaba", # 0
                "Nasilsin", # 1
                "İyiyim", # 2
                "Kotuyum", # 3
                "Seni", # 4
...
def check(param):
    ...
    #temp = model.predict(param)
            
    k = 0
    for i in range(2):
        #if model.predict(gray) == temp:
            # k++
        time.sleep(0.5)
        if k == 4 :
            return True
...
def main():
    ...
    while True:
        ...
        if pose_landmarks is not None:
            ...
            left, top, right, bottom = brect  # rectangle kordinatları
            #print(left, top, right, bottom)
            pose = Image.fromarray(image2, 'RGB') 
            pose = pose.crop((left-50, top-300, right+50, bottom+50)) # frame i crop ettim
            
            pose=np.array(pose) #np.uint8() # Hata önlemek ve modele uygun veri hali
            
            global gray
            
            gray = preProcess(pose)
            
            
            
#            gray = cv.cvtColor(pose, cv.COLOR_BGR2GRAY)
#            gray = cv.adaptiveThreshold(gray,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY,11,2)
            
            
            
            img = cv.resize(gray, (224, 224)) # input shape
           
            img = img.reshape(1,224, 224,1) # 1 adet resim, (224, 224) boytunda, channel = 1
            
            cv.imwrite("{}.jpg".format(2), gray)
            
            cv.imshow("Vucut",gray)
            ...
            if probVal > 0.5:
                cv.putText(gray, str(predictList[classIndex])+"     " + str(probVal),(50,50),cv.FONT_HERSHEY_SIMPLEX,1,(0,255,0),1,cv.LINE_AA)
...
#            print(predictList[classIndex],probVal)
        ...
        if left_hand_landmarks is not None:
            ...
            left, top, right, bottom = brect  # rectangle kordinatları
            #print(left, top, right, bottom)
            rightHand = Image.fromarray(image2, 'RGB') 
            rightHand = rightHand.crop((left-50, top-50, right+50, bottom+50)) # frame i crop ettim
            
            rightHand=np.array(rightHand) #np.uint8() # Hata önlemek ve modele uygun veri hali

            grayRightHand = cv.cvtColor(rightHand, cv.COLOR_BGR2GRAY)
            grayRightHand = cv.adaptiveThreshold(grayRightHand,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY,11,2)
            
            
#            hsvim = cv.cvtColor(rightHand, cv.COLOR_BGR2HSV)
#            lower = np.array([0, 48, 80], dtype = "uint8")
#            upper = np.array([20, 255, 255], dtype = "uint8") # thresh hold işlemleri
#            skinRegionHSV = cv.inRange(hsvim, lower, upper)
#            blurred = cv.blur(skinRegionHSV, ksize=(2,2))
#            ret,thresh = cv.threshold(blurred,0,255,cv.THRESH_BINARY)

            
            #cv.imshow("Sag El",grayRightHand)
        ...
        if right_hand_landmarks is not None:
            ...
            left, top, right, bottom = brect  # rectangle kordinatları
            #print(left, top, right, bottom)
            leftHand = Image.fromarray(image2, 'RGB') 
            leftHand = leftHand.crop((left-50, top-50, right+50, bottom+50)) # frame i crop ettim
            
            leftHand=np.array(leftHand) #np.uint8() # Hata önlemek ve modele uygun veri hali
            
            grayLeftHand = cv.cvtColor(leftHand, cv.COLOR_BGR2GRAY)
            grayLeftHand = cv.adaptiveThreshold(grayLeftHand,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY,11,2)
            
#            rightHand = cv.cvtColor(rightHand, cv.COLOR_BGR2GRAY)
#            rightHand = cv.adaptiveThreshold(rightHand,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY,11,2)
#            rightHand = cv.GaussianBlur(rightHand,ksize=(3,3),sigmaX=7)
            
            
            
#            hsvim = cv.cvtColor(leftHand, cv.COLOR_BGR2HSV)
#            lower = np.array([0, 48, 80], dtype = "uint8")
#            upper = np.array([20, 255, 255], dtype = "uint8") # thresh hold işlemleri
#            skinRegionHSV = cv.inRange(hsvim, lower, upper)
#            blurred = cv.blur(skinRegionHSV, ksize=(2,2))
